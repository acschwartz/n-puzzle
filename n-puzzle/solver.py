...
#        from wrapper import printException, lineno
#        printException(exc, lineno())
         


    if not data:
        return (None, None, 0)  # (None, None, None) throws an exception when you try to unpack it
...
#        print(color('red', 'peak memory use (calculated): '), bytes_to_human_readable_string(peak))
    print(color('black2', f' \u231B search duration: {prettyTime(t_search)}'))
...
#    print(fmt % (complexity['time'], t_search / max(complexity['time'],1) ))
    if searchTimedOut:
        print(color('red2', f' \N{ALARM CLOCK} Search timed out after {prettyTime(TIMEOUT_SEC)} !)'))
    if success:
